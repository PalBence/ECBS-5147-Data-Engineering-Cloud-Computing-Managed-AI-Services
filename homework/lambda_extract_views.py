...
def lambda_handler(event, context):
    ...
    all_views = response.json()["items"][0]["articles"]
    current_time = datetime.datetime.now(datetime.timezone.utc)

    json_lines = ""
    for page in all_views:
        record = {
            "title": page["article"],
            "views": page["views"],
            "rank": page["rank"],
    ...
    return {
        "statusCode": 200,
        "body": f"Uploaded {len(all_views)} records to s3://{S3_WIKI_BUCKET}/{s3_key}",
    }
